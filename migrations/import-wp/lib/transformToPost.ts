...
type StagedPost = Omit<Archive, '_createdAt' | '_updatedAt' | '_rev'>
...
export async function transformToPost(
  ...
  else {  // Make the first image the featured image
    const firstImageRef = doc.content?.find((block) => block._type === 'image')?.asset?._ref
    if (firstImageRef) {
      doc.featuredMedia = sanityIdToImageReference(firstImageRef)
    }
  }
